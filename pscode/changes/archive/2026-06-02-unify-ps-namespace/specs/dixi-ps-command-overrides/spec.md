...
**Reason**: O namespace `/pstld:*` é eliminado (ver capability `ps-command-unification`).
**Migration**: `arch-check`, `adr`, `dod` e `jira-draft` são absorvidos pelos overrides `/ps:*` (`apply`, `propose`, `complete`, `draft`); `installDixiCommands` deixa de copiar para `.claude/commands/pstld/`.

### Requirement: Arquivos de conteúdo Dixi-aware empacotados com o pscode
**Reason**: A estrutura de conteúdo muda — o diretório `pscode/content/dixi/commands/pstld/` é removido e os overrides `/ps:*` passam a embutir as capacidades absorvidas.
**Migration**: Os arquivos de conteúdo Dixi-aware passam a viver apenas em `pscode/content/dixi/commands/ps/` (incluindo `complete.md` e `board-setup.md`); `pstld/` é deletado.

## MODIFIED Requirements

### Requirement: Override dixi prefixa o ID do ticket no título do PR
Os overrides `/ps:propose` e `/ps:apply` do perfil `dixi` SHALL instruir, de forma
chumbada (sem chave de config e sem pergunta no init), que ao abrir o PR o título
seja prefixado com `[<jiraIssueKey>]` do metadata da change, e que a linha de link do
Trello no corpo seja suprimida.
