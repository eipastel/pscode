...
Define que o perfil `dixi` gera todos os artefatos de saída sob o diretório padrão `pscode/` (valor de `PSCODE_DIR_NAME`), nunca sob o nome legado `pastelsdd/`, e que a ferramenta migra de forma não-destrutiva repositórios que ainda usam o diretório legado.
