...
Quando um skill de apply (`/ps:apply`) é executado, o sistema SHALL incluir as instruções de PR no contexto do agente se `pr.enabled` for `true`.

#### Scenario: Apply com PR habilitado
- **WHEN** o usuário executa `/ps:apply` em um projeto com `pr.enabled: true`
...
- **THEN** o agente é instruído a criar uma branch com o padrão configurado antes de implementar
