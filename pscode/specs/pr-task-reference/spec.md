...
Define como o workflow de PR embute a referência da task do tracker dentro do Pull Request aberto automaticamente, de forma idiomática por perfil: link do card do Trello no corpo do PR (perfil `standard`) e `[ID]` do ticket no título do PR (perfil `dixi`).
