# Spec: pstld-jira-context-skill

## Purpose

Skill que injeta automaticamente contexto de tickets JIRA no prompt quando detecta uma chave no formato `[A-Z]+-\d+`, usando a integração MCP Atlassian quando configurada em `pastelsdd/jira.yaml`.
