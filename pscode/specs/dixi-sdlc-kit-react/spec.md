...
Defines the React/Next.js-specific SDLC kit files installed by `installDixiExtras` when the detected project family is `react`. Covers editor configuration, Husky hooks (commit-msg + pre-commit), lint-staged config, and GitHub Actions CI pipeline for React/TypeScript projects.
