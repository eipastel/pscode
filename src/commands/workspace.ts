...
async function promptWorkspaceSkillAgents(
  ...
  return searchableMultiSelect({
    message: 'Which agents should get Pscode skills in this workspace?',
    pageSize: 15,
...
function printWorkspaceListHuman(workspaces: WorkspaceListOutput[]): void {
  console.log(chalk.bold(`Pscode workspaces (${workspaces.length})`));
...
function printWorkspaceOpenHuman(prepared: PreparedWorkspaceOpen): void {
  ...
  console.log('Repair skipped links with pscode workspace doctor.');
}
...
class WorkspaceCommand {
    ...
    try {
      ...
      if (options.json && !noInteractive) {
        throw new WorkspaceCliError(
          ...
          {
            fix: 'pscode workspace setup --no-interactive --json --name <name> --link <path>',
          }
        );
      }
      ...
      if (!interactive && (!options.name || (options.link ?? []).length === 0)) {
        throw new WorkspaceCliError(
          ...
          {
            fix: 'pscode workspace setup --no-interactive --name platform --link /path/to/repo',
          }
        );
      }
      ...
      if (options.tools !== undefined) {
        ...
        console.log(chalk.dim('Choose which coding agents should get Pscode skills in this workspace.'));
      ...
      const skillReport =
        selectedWorkspaceSkillAgents === undefined
          ? createWorkspaceSkillSkippedReport(
              'tools_omitted',
              'No workspace skills were installed. Run pscode workspace update --tools <ids> to install them later.'
            )
      ...
      console.log(`  pscode workspace doctor --workspace ${workspace.name}`);
      console.log(`  pscode workspace update --workspace ${workspace.name} --tools <ids>`);
      console.log('  pscode workspace list');
  ...
  async list(options: WorkspaceListOptions = {}): Promise<void> {
    try {
      ...
      if (workspaces.length === 0) {
        console.log("No Pscode workspaces found. Run 'pscode workspace setup' first.");
        return;
      }
    ...
    try {
      if (!nameOrPath) {
        throw new WorkspaceCliError(
          ...
          'missing_link_path',
          {
            fix: 'pscode workspace link /path/to/repo',
          }
        );
      }
    ...
    try {
      if (!linkNameInput || !linkPath) {
        throw new WorkspaceCliError(
          ...
          {
            fix: 'pscode workspace relink <name> /path/to/repo',
          }
        );
      }
  ...
  private async updateSelected(
    ...
    console.log('');
    console.log('Next useful commands:');
    console.log(`  pscode workspace doctor --workspace ${doctorResult.workspace.name}`);
    console.log(`  pscode workspace update --workspace ${doctorResult.workspace.name} --tools <ids>`);
