 ...
 * - Helps understand Pscode's data interfaces at a glance
 * - Single source of truth for type and validation
 * - Consistent with other Pscode schemas
 */
...
/**
 * Read and parse pscode/config.yaml from project root.
 ...
 *
 * @param projectRoot - The root directory of the project (where `pscode/` lives)
 * @returns Parsed config or null if file doesn't exist
 */
export function readProjectConfig(projectRoot: string): ProjectConfig | null {
  // Try both .yaml and .yml, prefer .yaml
  let configPath = path.join(projectRoot, 'pscode', 'config.yaml');
  if (!existsSync(configPath)) {
    configPath = path.join(projectRoot, 'pscode', 'config.yml');
    if (!existsSync(configPath)) {
  ...
  try {
    ...
    if (!raw || typeof raw !== 'object') {
      console.warn(`pscode/config.yaml is not a valid YAML object`);
      return null;
    }
    ...
    console.warn(`Failed to parse pscode/config.yaml:`, error);
    return null;
  }
}
...
export function suggestSchemas(
  ...
  let message = `Schema '${invalidSchemaName}' not found in pscode/config.yaml\n\n`;
  ...
  message += `\nFix: Edit pscode/config.yaml and change 'schema: ${invalidSchemaName}' to a valid schema name`;
