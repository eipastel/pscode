...
describe('initiative command', () => {
  ...
  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'pscode-initiative-command-'));
  ...
  it('prints readable human output for create and list', async () => {
    ...
    expect(create.stdout).toContain('pscode initiative list --store team-context');
    ...
    expect(list.exitCode).toBe(0);
    expect(list.stdout).toContain('Pscode initiatives in team-context (1)');
  ...
  it('prints compact all-store human output without initiative statuses', async () => {
    ...
    expect(list.exitCode).toBe(0);
    expect(list.stdout).toContain('Pscode initiatives (1 across 2 stores)');
  ...
  it('prints compact human output for initiative show', async () => {
    ...
    expect(show.exitCode).toBe(0);
    expect(show.stdout).toContain('Pscode initiative: Billing Launch');
  ...
  it('reports all-store empty and partial-read initiative list states', async () => {
    ...
    expect(partialPayload.status[0]).toEqual(
      expect.objectContaining({
        ...
        fix: 'pscode context-store doctor',
      })
    );
