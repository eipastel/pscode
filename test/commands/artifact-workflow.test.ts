...
describe('artifact-workflow CLI commands', () => {
  ...
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'pscode-artifact-workflow-'));
    changesDir = path.join(tempDir, 'pscode', 'changes');
  ...
  describe('status command', () => {
    ...
    it('exits gracefully when no changes exist', async () => {
      ...
      expect(result.stdout).toContain('pscode new change');
    });
  ...
  describe('new change command', () => {
    ...
    it('creates workspace-planning changes under the workspace root without touching linked repos', async () => {
      ...
      await fs.mkdir(path.join(api, 'pscode', 'specs'), { recursive: true });
      ...
      const metadata = await fs.readFile(path.join(changeDir, '.pscode.yaml'), 'utf-8');
      ...
      await expect(fs.stat(path.join(api, 'pscode', 'changes'))).rejects.toMatchObject({
        code: 'ENOENT',
      });
    });
  ...
  describe('instructions apply command', () => {
    ...
    it('resolves single-star glob artifacts consistently between status and apply', async () => {
      const schemaDir = path.join(tempDir, 'pscode', 'schemas', 'glob-test');
...
`
      ...
      await fs.mkdir(path.dirname(specPath), { recursive: true });
      await fs.writeFile(path.join(changeDir, '.pscode.yaml'), 'schema: glob-test\n');
    ...
    it('fallback: requires all artifacts when schema has no apply block', async () => {
      ...
      const noApplySchemaDir = path.join(userDataDir, 'pscode', 'schemas', 'no-apply');
...
`;
    ...
    it('fallback: ready when all artifacts exist for schema without apply block', async () => {
      ...
      const noApplySchemaDir = path.join(userDataDir, 'pscode', 'schemas', 'no-apply-full');
      const templatesDir = path.join(noApplySchemaDir, 'templates');
      await fs.mkdir(templatesDir, { recursive: true });
...
`;
  ...
  describe('experimental command (deprecated alias for init)', () => {
    ...
    it('creates skills for Claude tool', async () => {
      ...
      const skillFile = path.join(tempDir, '.claude', 'skills', 'pscode-explore', 'SKILL.md');
    ...
    it('creates skills for Cursor tool', async () => {
      ...
      // Verify skill files were created
      const skillFile = path.join(tempDir, '.cursor', 'skills', 'pscode-explore', 'SKILL.md');
      const stat = await fs.stat(skillFile);
      expect(stat.isFile()).toBe(true);

      // Verify commands were created with Cursor format
      const commandFile = path.join(tempDir, '.cursor', 'commands', 'ps-explore.md');
      const content = await fs.readFile(commandFile, 'utf-8');
      expect(content).toContain('name: /ps-explore');
    });
    ...
    it('creates skills for Gemini tool', async () => {
      ...
      // Verify skill files were created
      const skillFile = path.join(tempDir, '.gemini', 'skills', 'pscode-explore', 'SKILL.md');
      const stat = await fs.stat(skillFile);
      expect(stat.isFile()).toBe(true);
    });
  });
  ...
  describe('project config integration', () => {
    describe('new change uses config schema', () => {
      it('creates change with schema from project config', async () => {
        ...
        // Note: changesDir is already at tempDir/pscode/changes (created in beforeEach)
        await fs.writeFile(
          path.join(tempDir, 'pscode', 'config.yaml'),
          'schema: spec-driven\n'
        );
        ...
        const metadataPath = path.join(changesDir, 'test-change', '.pscode.yaml');
      ...
      it('CLI schema overrides config schema', async () => {
        // Create project config with spec-driven schema
        // Note: pscode directory already exists (from changesDir creation in beforeEach)
        await fs.writeFile(
          path.join(tempDir, 'pscode', 'config.yaml'),
          'schema: spec-driven\n'
        );
        ...
        const metadataPath = path.join(changesDir, 'override-test', '.pscode.yaml');
        const metadata = await fs.readFile(metadataPath, 'utf-8');
        expect(metadata).toContain('schema: spec-driven');
      }, 60000);
    });
