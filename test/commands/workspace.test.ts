  ...
  it('installs profile-selected workspace skills in the workspace root only', async () => {
    ...
    expect(payload.workspace_skills).toEqual(
      expect.objectContaining({
        profile: 'standard',
        delivery: 'commands',
        workflow_ids: ['propose', 'explore', 'apply', 'complete', 'draft', 'handoff', 'board-setup', 'github-setup'],
    ...
    expect(fs.existsSync(path.join(workspaceRoot, '.codex', 'skills', 'pscode-complete-change', 'SKILL.md'))).toBe(true);
    ...
    expect(readWorkspaceState(workspaceRoot).workspace_skills).toEqual(
      expect.objectContaining({
        ...
        last_applied_workflow_ids: ['propose', 'explore', 'apply', 'complete', 'draft', 'handoff', 'board-setup', 'github-setup'],
