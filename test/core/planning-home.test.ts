...
describe('planning home paths', () => {
  ...
  it('keeps a canonical workspace root comparable with an aliased start path', () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'pscode-planning-home-'));
    tempDirs.push(tempDir);
    const realWorkspaceRoot = path.join(tempDir, 'real-workspace');
    const aliasWorkspaceRoot = path.join(tempDir, 'alias-workspace');

    fs.mkdirSync(path.join(realWorkspaceRoot, '.pscode-workspace'), { recursive: true });
    fs.writeFileSync(
      path.join(realWorkspaceRoot, '.pscode-workspace', 'workspace.yaml'),
      'version: 1\nname: platform\nlinks: {}\n',
      'utf-8'
    );
    fs.symlinkSync(
  ...
  it('surfaces invalid current workspace state instead of falling back to legacy state', () => {
    ...
    fs.mkdirSync(path.join(workspaceRoot, '.pscode-workspace'), { recursive: true });
    fs.writeFileSync(
    ...
    fs.writeFileSync(
      path.join(workspaceRoot, '.pscode-workspace', 'workspace.yaml'),
