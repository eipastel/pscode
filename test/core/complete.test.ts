...
describe('CompleteCommand', () => {
  ...
  beforeEach(async () => {
    ...
    // Create Pscode structure
    const pscodeDir = path.join(tempDir, 'pscode');
    await fs.mkdir(path.join(pscodeDir, 'changes'), { recursive: true });
    await fs.mkdir(path.join(pscodeDir, 'specs'), { recursive: true });
    await fs.mkdir(path.join(pscodeDir, 'changes', 'archive'), { recursive: true });
...
### Requirement: Thumbnail Field`;
      ...
      const mainSpecPath = path.join(tempDir, 'pscode', 'specs', 'gift-card', 'spec.md');
      const updatedContent = await fs.readFile(mainSpecPath, 'utf-8');
...
Modified content.`;
      ...
      const mainSpecPath = path.join(tempDir, 'pscode', 'specs', 'new-capability', 'spec.md');
...
- TO: \`### Requirement: New Name\``;
      ...
      // Verify spec was NOT created
      const mainSpecPath = path.join(tempDir, 'pscode', 'specs', 'another-capability', 'spec.md');
      await expect(fs.access(mainSpecPath)).rejects.toThrow();
    ...
    it('should throw error if archive already exists', async () => {
      ...
      const archivePath = path.join(tempDir, 'pscode', 'changes', 'archive', `${date}-${changeName}`);
...
- **THEN** specs are synced automatically`;
    ...
    it('should support header trim-only normalization for matching', async () => {
      ...
      const mainSpecDir = path.join(tempDir, 'pscode', 'specs', 'alpha');
...
Updated details.`;
    ...
    it('should apply operations in order: RENAMED → REMOVED → MODIFIED → ADDED', async () => {
      ...
      const mainSpecDir = path.join(tempDir, 'pscode', 'specs', 'beta');
      await fs.mkdir(mainSpecDir, { recursive: true });
...
content D`;
    ...
    it('should abort with error when MODIFIED/REMOVED reference non-existent requirements', async () => {
      ...
      const mainSpecDir = path.join(tempDir, 'pscode', 'specs', 'gamma');
      await fs.mkdir(mainSpecDir, { recursive: true });
...
### Requirement: Another Missing`;
    ...
    it('should abort with a structural error when target spec hides requirements outside ## Requirements', async () => {
      ...
      const mainSpecDir = path.join(tempDir, 'pscode', 'specs', 'delta-target');
      await fs.mkdir(mainSpecDir, { recursive: true });
...
- **THEN** qux changes`;
    ...
    it('should require MODIFIED to reference the NEW header when a rename exists (error format)', async () => {
      ...
      // Main spec with Old
      const mainSpecDir = path.join(tempDir, 'pscode', 'specs', 'delta');
      await fs.mkdir(mainSpecDir, { recursive: true });
...
new body`;
    ...
    it('should process multiple specs atomically (any failure aborts all)', async () => {
      ...
      // Existing main specs
      const epsilonMain = path.join(tempDir, 'pscode', 'specs', 'epsilon', 'spec.md');
...
e1`);
      ...
      const zetaMain = path.join(tempDir, 'pscode', 'specs', 'zeta', 'spec.md');
...
### Requirement: Missing`);
    ...
    it('should display aggregated totals across multiple specs', async () => {
      ...
      // Existing main specs
      const omegaMain = path.join(tempDir, 'pscode', 'specs', 'omega', 'spec.md');
      await fs.mkdir(path.dirname(omegaMain), { recursive: true });
      await fs.writeFile(omegaMain, `# omega Specification\n\n## Purpose\nOmega purpose.\n\n## Requirements\n\n### Requirement: O1\no1`);

      const psiMain = path.join(tempDir, 'pscode', 'specs', 'psi', 'spec.md');
  ...
  describe('error handling', () => {
    it('should throw error when pscode directory does not exist', async () => {
  ...
  describe('interactive mode', () => {
    it('should use select prompt for change selection', async () => {
      ...
      await fs.mkdir(path.join(tempDir, 'pscode', 'changes', change1), { recursive: true });
      await fs.mkdir(path.join(tempDir, 'pscode', 'changes', change2), { recursive: true });
