...
describe('PowerShellGenerator', () => {
	...
	describe('generate', () => {
		it('should generate valid PowerShell completion script with header', () => {
			...
			expect(script).toContain('# PowerShell completion script for Pscode CLI');
			expect(script).toContain('$pscodeCompleter = {');
			expect(script).toContain('Register-ArgumentCompleter');
		});

		it('should register argument completer for pscode command', () => {
			const commands: CommandDefinition[] = [
				{
					name: 'init',
					description: 'Initialize Pscode',
			...
			expect(script).toContain('Register-ArgumentCompleter -CommandName pscode');
			expect(script).toContain('-ScriptBlock $pscodeCompleter');
		});
		...
		it('should handle positional arguments for change-id', () => {
			...
			expect(script).toContain('Get-PscodeChanges');
		});
		...
		it('should handle positional arguments for spec-id', () => {
			...
			expect(script).toContain('Get-PscodeSpecs');
		});
		...
		it('should handle positional arguments for schema names', () => {
			...
			expect(script).toContain('Get-PscodeSchemas');
			expect(script).toContain('pscode __complete schemas 2>$null');
		});
		...
		it('should generate dynamic completion helper for changes', () => {
			...
			expect(script).toContain('function Get-PscodeChanges');
			expect(script).toContain('pscode __complete changes 2>$null');
		...
		it('should generate dynamic completion helper for specs', () => {
			...
			expect(script).toContain('function Get-PscodeSpecs');
			expect(script).toContain('pscode __complete specs 2>$null');
		});
